...
- **データ取得を行う画面は `useAuth()` の `authLoading` を待ち、依存配列にも
  含めること。** これがないと、非デフォルトタブ（`/collection`など）への
  直接ハードリロード時に `useFocusEffect` の `'focus'` イベントを
  取りこぼしたまま二度と再取得されない（`9aaefe9`）。デフォルトタブ
  （`/`）はマウント時点で既にフォーカス済みなので発生しない。
  `authLoading` を依存に含めると、トークン復元完了時に再実行される
  「取りこぼしの救済」を兼ねる。新しい画面を追加するときは
  `(tabs)/index.tsx`・`useMapMarkers.ts` と同じ形になっているか確認する。
